...
pub async fn post_by_slug(
    ...
    addr.send(FindBy::slug(slug.to_string()))
        .await
        .unwrap()
        .map(|res| {
            res.map(|post| match post {
                Post::Post {
                    markdown_content,
                    title,
                    current_slug,
            ...
            .unwrap_or_else(|| HttpResponse::NotFound().json(""))
        })
...
pub async fn index(addr: web::Data<Addr<PgActor>>) -> Result<HttpResponse, Error> {
    addr.send(Find::posts())
        .await
        .unwrap()
        .map(|res| {
            let posts = res.iter().map(PostSummaryView::for_human).collect();
...
pub async fn feed(addr: web::Data<Addr<PgActor>>) -> Result<HttpResponse, Error> {
    addr.send(Find::posts())
        .await
        .unwrap()
        .map(|res| {
            let posts = res.iter().map(PostSummaryView::for_machine).collect();
